...
RSpec.describe 'Finds the Merchant with the most items' do
  scenario 'returns x number top merchants with the most items sold' do
    ...
    create(:invoice_item, quantity: 12, unit_price: 101.00, invoice_id: invoice1.id, item_id: create(:item, unit_price: 10.00).id)

    invoice2 = create(:invoice, merchant_id: merchant1.id, status: 'shipped')
    create(:transaction, result: 'failed', invoice_id: invoice2.id)
    create(:invoice_item, quantity: 5, unit_price: 10.00, invoice_id: invoice2.id, item_id: create(:item, unit_price: 10.00).id)

    invoice3 = create(:invoice, merchant_id: merchant2.id, status: 'shipped')
    create(:transaction, result: 'success', invoice_id: invoice3.id)
    create(:invoice_item, quantity: 6, unit_price: 2_103.00, invoice_id: invoice3.id, item_id: create(:item, unit_price: 20.00).id)

    invoice4 = create(:invoice, merchant_id: merchant2.id, status: 'shipped')
    create(:transaction, result: 'failed', invoice_id: invoice4.id)
    create(:invoice_item, quantity: 11, unit_price: 20.00, invoice_id: invoice4.id, item_id: create(:item, unit_price: 20.00).id)

    invoice5 = create(:invoice, merchant_id: merchant1.id, status: 'shipped')
    create(:transaction, result: 'success', invoice_id: invoice5.id)
    create(:invoice_item, quantity: 2, unit_price: 345.00, invoice_id: invoice5.id, item_id: create(:item, unit_price: 30.00).id)

    invoice6 = create(:invoice, merchant_id: merchant3.id, status: 'shipped')
    create(:transaction, result: 'failed', invoice_id: invoice6.id)
    create(:invoice_item, quantity: 7, unit_price: 375.00, invoice_id: invoice6.id, item_id: create(:item, unit_price: 30.00).id)

    invoice7 = create(:invoice, merchant_id: merchant2.id, status: 'packaged')
    create(:transaction, result: 'success', invoice_id: invoice7.id)
    create(:invoice_item, quantity: 1, unit_price: 430.34, invoice_id: invoice7.id, item_id: create(:item, unit_price: 40.00).id)
    ...
    create(:invoice_item, quantity: 1, unit_price: 500.00, invoice_id: invoice9.id, item_id: create(:item, unit_price: 50.00).id)
    ...
    create(:invoice_item, quantity: 4, unit_price: 500.00, invoice_id: invoice11.id, item_id: create(:item, unit_price: 50.00).id)
    ...
    results.each do |result|
      expect(result[:id].to_i).to eq(merchant1.id).or(eq merchant2.id).or(eq merchant5.id)
      expect(result[:id].to_i).to_not eq(merchant3.id)
      expect(result[:id].to_i).to_not eq(merchant4.id)
      expect(result[:attributes][:name]).to eq(merchant1.name).or(eq merchant2.name).or(eq merchant5.name)
    end
  end
end
